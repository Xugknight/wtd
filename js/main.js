(function () {
    ...
    applyUIToToggles();
    if (ui.mdPreview) { mdPreview.classList.remove('hidden'); renderMarkdown(); }
    ...
    function renderTabs() {
        if (!tabsEl) return;
        // clear tabs except + and any more-menu
        tabsEl.querySelectorAll('.tab:not(.add):not(.more)').forEach(n => n.remove());
        // simple overflow: show first 7 as tabs, rest in a dropdown
        const VISIBLE = 7;
...
<html><head><meta charset="utf-8"><title>${safeName(active().name)} — Export</title></head><body>${inner}</body></html>`;
    ...
    function colorMix(varName, pct) {
        return `color-mix(in srgb, var(${varName}), transparent ${pct}%)`;
    }
